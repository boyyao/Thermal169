...
// 调整顺序到前面 calculate_pixel_temp需要调用
/********************************************************************
...
/********************************************************************
   ...
   Dependencies:    register address (addr),
                    number of bytes (n)
 *******************************************************************/
bool write_sensor_byte(uint8_t deviceaddress, uint8_t registeraddress, uint8_t input)
{
...
/********************************************************************
   ...
   Dependencies:    register address (address)
 *******************************************************************/
bool write_EEPROM_byte(unsigned short address, unsigned char content)
{
  ...
  ******************************************************************************************************************/
  ...
  for (int m = 0; m < DevConst.PixelPerColumn; m++)
  {
    for (int n = 0; n < DevConst.PixelPerRow; n++)
    {
      ...
      Serial.print(pixel);
      Serial.print("\t");
      Serial.print((float)((pixel - 2732) / 10.0));
      Serial.print("\n");
      /******************************************************************************************************************
        step 8: overwrite the uncompensate pixel with the new calculated compensated value
      ******************************************************************************************************************/
...
void sensor_setup()
{
  ...
  // pixc2_0 = (unsigned long*)heap_caps_malloc(NUMBER_OF_PIXEL * 4, MALLOC_CAP_32BIT); //esp
  ...
  uint8_t error = 1; // 默认值不一样。需一个非0的值
  while (error != 0)
  {
    delay(2000);
    Wire.begin();
...
void sensor_loop()
{
  ...
  { // state is 1 when all raw sensor voltages are read for this picture
    waitForUnlock(pix_cp_lock, 1);
    {
      LockGuard lock(prob_lock);
...
void setfsl(unsigned char newepsilon)
{
  ReadingRoutineEnable = 0;
  //  TimerLib.clearTimer();
  Serial.println("\n\n\n---Increase emissivity---");
  ...
  Serial.println(epsilon);
  Serial.print("new emissivity: \t");
  if (newepsilon < 100 & newepsilon > 0)
  {
    epsilon = newepsilon;
    Wire.setClock(CLOCK_EEPROM); // I2C clock frequency 400kHz (for eeprom communication)
    write_EEPROM_byte(E_EPSILON, epsilon);
